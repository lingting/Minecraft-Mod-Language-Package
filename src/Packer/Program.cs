...
namespace Packer
{
    class Program
    {
        ...
        public static async Task Main(string version, bool increment = false)
        {
            ...
            var targetModIdentifiers = increment
                ? GitHelpers.EnumerateChangedMods(config.Base.Version)
            ...
            var query = // 在这里加日志似乎开销太大了。暂时注释掉
                ...
                group provider by provider.Destination
            ...
            string md5Name = $"./Minecraft-Mod-Language-Package-{config.Base.Version}.md5";
            ...
            Log.Information("对版本 {0} 的打包结束。文件: {1}", version, packName);
            Log.Information("资源包文件: {0}", packName);

            var md5 = stream.ComputeMD5();
            Log.Information("打包文件的 MD5 值：{0}", md5);
            File.WriteAllText(md5Name, md5);
            Log.Information("md5文件: {0}", md5Name);
        }
    }
}